...
class Tests_Session extends MZMBOAccess_WPUnitTestCase {
	...
	function setUp() {
		...
		$this->el( dbDelta( $sql ) );
		new MZoo\MzMindbody\Session\MzPhpSession();
	}
	...
	public function test_set() {
		$this->assertEquals( '"bar"', MZoo\MzMindbody\Session\MzPhpSession::instance()->set( 'foo', 'bar' ) );
	}

	public function test_get() {
		$this->assertEquals( 'bar', MZoo\MzMindbody\Session\MzPhpSession::instance()->get( 'foo' ) );
	}
	...
	public function test_should_start_session() {
		$blacklist = MZoo\MzMindbody\Session\MzPhpSession::instance()->get_blacklist();
		foreach ( $blacklist as $uri ) {
			$this->go_to( '/' . $uri );
			$this->assertFalse( MZoo\MzMindbody\Session\MzPhpSession::instance()->should_start_session() );
		}
	}
}
