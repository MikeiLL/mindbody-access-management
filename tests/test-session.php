...
class Tests_Session extends MZMBOAccess_WPUnitTestCase
{
    ...
    function setUp()
    {
        ...
        $this->el(dbDelta($sql));
        new MzMboAccess\Session\MzAccessSession();
    }
    ...
    {
        $this->assertEquals('"bar"', MzMboAccess\Session\MzAccessSession::instance()->set('foo', 'bar'));
    }

    public function test_get()
    {
        $this->assertEquals('bar', MzMboAccess\Session\MzAccessSession::instance()->get('foo'));
    }
    ...
    {
        $blacklist = MzMboAccess\Session\MzAccessSession::instance()->get_blacklist();
        foreach ($blacklist as $uri) {
            $this->go_to('/' . $uri);
            $this->assertFalse(MzMboAccess\Session\MzAccessSession::instance()->should_start_session());
        }
    }
}
